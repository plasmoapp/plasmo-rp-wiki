...
**Как создать общину:** [https://rp.plo.su/wiki/commune/](https://rp.plo.su/wiki/commune/)
...
```
НонРП персонаж говорит: "Кпидс идиот".
...
Если РП персонаж хочет сказать о чём-то связанном с реальным миром, то он может использовать нонРП чат, если обнесёт сообщение (( скобочками )) или использует команду `/b` или префикс `.<сообщение>`
...
Глобальные РП команды можно использовать, например, для вызова Интерпола `/gme вызывает интерпол на метку :m123:, подозрение на кражу.` Интерпол, который отвечает на вызов, должен написать `/gme принял вызов <игрок>`
...
**Ищите метки тут:** [https://rp.plo.su/map/](https://rp.plo.su/map/)

`/x search <запрос>` — найти метку по запросу в игре

`/x favorite <номер метки>` — добавить метку в избранное в меню меток
...
`/dynmap hide`, чтобы скрыть себя на карте, `/dynmap show`, чтобы показать себя на карте
...
**Узнать больше и скачать мод:** [https://rp.plo.su/wiki/mods/](https://rp.plo.su/wiki/mods/)
...
Чтобы сменить ник, нажмите ПКМ по иконке Discord сервера, а затем выберете "Изменить ник". Cоблюдайте правильный регистр, введите новый ник и сохраните его
...
<summary>Редактирование строек</summary>
